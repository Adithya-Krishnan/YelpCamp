...
module.exports.createCampground=async (req, res, next) => {
    ...
    campground.images =  req.files.map(f=>({url:f.path , filename:f.filename}))    //NEW CAMP POST ROUTE
...
module.exports.renderEditForm=async (req, res, next) => {
    const {id}=req.params;
    const campground = await Campground.findById(id)        //EDIT PAGE
    if(!campground){
...
module.exports.updateCampground=async (req, res) => {
    const { id } = req.params;                                      //UPDATE CAMP
...
module.exports.deleteCampground=async (req, res, next) => {
    const { id } = req.params;                                       //DELETE CAMP
